...
export async function getSixMonthForecast(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Fetch custom projects (blocks crew capacity for a date range)
    ...
    // Fetch pipeline totals (shingle + metal separately)
    ...
    for (const row of pipelineResult.rows) {
      pipelineMap[row.job_type] = parseFloat(row.total_sqs) || 0;
    }
    let rollingShingle = pipelineMap['shingle'] || 0;
    let rollingMetal   = pipelineMap['metal']   || 0;

    // Fetch sales forecasts for the window
    ...
    for (let i = 0; i < weeks; i++) {
      ...
      // Calculate production rates per type for this week
      let prodShingle = 0;
      let prodMetal   = 0;
      ...
      for (const crew of crews) {
        ...
        // Is crew active this week?
        if (crewStart > weekEnd) continue; // Not started yet
        if (crewEnd && crewEnd < weekStart) continue; // Already terminated

        weekCrewIds.add(crew.id);

        // Ramp-up multiplier: if crew started recently, capacity is partial
        ...
        // Ramp-down if crew terminates this week or next
        if (crewEnd) {
          const daysUntilEnd = Math.floor((crewEnd.getTime() - weekStart.getTime()) / (1000 * 60 * 60 * 24));
          if (daysUntilEnd < rampUpDays) {
            rampMultiplier = Math.min(rampMultiplier, daysUntilEnd / rampUpDays);
          }
        }

        // Check if crew is blocked by a custom project this week
      ...
      // Detect crew changes vs previous week
    ...
    res.json({ success: true, data: { weeks: weeklyData } });
  } catch (err) { next(err); }
}
